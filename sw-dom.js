document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sessionStorage.getItem('updated')) {
      ...
      caches.keys().then(cacheNames => {
        const cacheNameToDelete = "AnZhiYuBlogCache";
      
        if (cacheNames.includes(cacheNameToDelete)) {
          caches.delete(cacheNameToDelete).then(result => {
            if (result) {
              console.info('缓存删除成功');
            } else {
              console.info('缓存删除失败');
